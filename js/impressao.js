function gerarOrcamentoParaImpressao() {
  ...
  gruposContainers.forEach((container, index) => {
    ...
    if (!produtosPorAmbiente[nomeAmbiente]) {
      produtosPorAmbiente[nomeAmbiente] = [];
    }

    // 🔧 Coleta observação diretamente da célula td com classe editable-observacoes
    const observacao = (() => {
      const obsCelula = container.querySelector("td.editable-observacoes");
      return obsCelula?.textContent?.trim() || "";
    })();
    ...
    produtosPorAmbiente[nomeAmbiente].push({
      ...
      observacao,
      totalGrupo: produtosGrupo.reduce((acc, p) => acc + (parseFloat(p.cost || 0) * parseFloat(p.quantity || 0)), 0),
    });
  });

  Object.entries(produtosPorAmbiente).forEach(([ambiente, grupos], ambienteIndex) => {
    let totalAmbiente = 0;
    let blocos = "";

    grupos.forEach((grupoData, i) => {
      totalAmbiente += grupoData.totalGrupo;

      blocos += `
        <div class="parent border mb-0">
          <div class="div1 border p-2">Item ${i + 1}</div>
          <div class="div2 border p-2">${grupoData.descricao}</div>
          <div class="div3 border p-2">${grupoData.unidade}</div>
          <div class="div4 border p-2">${grupoData.nomeProduto}</div>
          <div class="div6 border p-2">${grupoData.observacao}</div>
        </div>
      `;
    });
